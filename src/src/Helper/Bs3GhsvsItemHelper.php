...
class Bs3GhsvsItemHelper
{
	...
	{
		$sizePostfixes = [];
		$what = Bs3GhsvsHelper::getPluginParams()->get($what);
	...
	public static function getImageResizeImages(
		...
		if ($collect_images)
		{
			...
			switch (Bs3GhsvsHelper::getPluginParams()->get('webpSupport'))
			{
				case 1:
	...
	public static function getAllImgSrc(
		...
		if (stripos($txt, '<img ') !== false)
		{
			...
			*/
			if (PluginHelper::isEnabled('system', 'bs3ghsvs'))
			{
				if (Bs3GhsvsHelper::getPluginParams()->get('imgSrcDetection')
					=== 'html4')
				{
	...
	*/
	...
	protected static function loadImageResizer()
	{
		...
		{
#echo ' 4654sd48sa7d98sD81s8d71dsa <pre>' . print_r(is_file(__DIR__ . '/../../Helper/ImgResizeCache.php'), true) . '</pre>';exit;
			\JLoader::register('ImgResizeCache', __DIR__ . '/../../Helper/ImgResizeCache.php');
			self::$imageResizer = new \ImgResizeCache(Bs3GhsvsHelper::getPluginParams());
		}
	}
